      ...
      <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        {((localStorage.getItem("role") as any) || "estudiante") ===
          ...
          <div className="flex gap-3">
            <Link
              to="/repositorio/gestionar/nuevo"
              state={{ from: location.pathname }}
              className="inline-flex items-center gap-1 rounded-xl bg-brand px-4 py-2.5 text-sm font-semibold text-white hover:bg-brand/90"
            >
              <FolderEdit className="h-4 w-4" /> Crear Nuevo Repositorio
            </Link>
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-neutral-600">
                    Creado: {r.fecha} • Estado: {r.estado}
                  </div>
                </div>
                <div className="flex gap-2">
                  <Link
                    to={`/repositorio/gestionar/${r.id}`}
                    state={{ from: location.pathname }}
                    className="rounded-lg px-3 py-1.5 text-sm font-medium text-contrast hover:bg-neutral-100"
                  >
                    Gestionar / Editar
                  </Link>
                  <button className="inline-flex items-center gap-1 rounded-lg px-3 py-1.5 text-sm font-medium text-brand hover:bg-brand/10">
            ...
            (import.meta.env.DEV ? (
              ...
              <div className="rounded-2xl border border-neutral-200 p-6 text-center">
                <div className="text-sm text-neutral-700">
                  No has creado ningún repositorio.
                </div>
                <Link
                  to="/repositorio/gestionar/nuevo"
                  className="mt-3 inline-flex items-center justify-center rounded-xl bg-brand px-4 py-2.5 text-sm font-semibold text-white hover:bg-brand/90"
                >
