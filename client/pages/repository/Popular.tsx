  ...
  const list = useMemo(() => {
    ...
    return base.filter(
      (r) =>
        ...
        r.docente.toLowerCase().includes(s) ||
        r.temas.some((t) => t.toLowerCase().includes(s)),
    );
  }, [q, data]);
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-neutral-600">
        Explora contenido educativo validado por la comunidad.
      </p>
      ...
      <div className="mt-4">
        <div className="relative mb-5">
          ...
          <input
            ...
            className="w-full rounded-xl border border-neutral-200 bg-white px-10 py-3 outline-none transition focus:border-contrast focus:ring-2 focus:ring-contrast/20"
          />
        </div>
              ...
              >
                ...
                <div className="mb-3 flex items-center gap-4 text-xs text-neutral-600">
                  <span className="inline-flex items-center gap-1">
                    <Download className="h-4 w-4" /> {r.descargas} descargas
                  </span>
                  <span className="inline-flex items-center gap-1">
                    <Eye className="h-4 w-4" /> {r.vistas} vistas
                  </span>
                </div>
                <Link
                  to={`/repositorio/ver/${r.id}`}
                  state={{ from: location.pathname }}
                  className="inline-flex items-center justify-center rounded-xl bg-brand px-4 py-2.5 text-sm font-semibold text-white hover:bg-brand/90"
                >
                  Ver Recursos
                </Link>
              </article>
